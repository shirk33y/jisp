...
export default function minimal(E: Env) {
  ...
  function evalLoop(ast: AstNode, env: Env): AstNode {
    while (true) {
      ...
      // if (ast[0] == "def") {
      //   // update current environment
      //   return (env[`${ast[1]}`] = evalLoop(ast[2], env));
      // } else if (ast[0] == "~") {
      //   // mark as macro
      //   const f = evalLoop(ast[1], env); // eval regular function
      //   // if(!Array.isArray(f)) throw Error(`Bad macro:, ${ast[1]}`)
      //   (f as any)["M"] = 1; // mark as macro
      //   return f;
      // } else if (ast[0] == "`") {
      //   // quote (unevaluated)
      ...
      //     return evalLoop(ast[1], env);
      //   } catch (e) {
      //     return evalLoop(at(ast, 2, 2), evalOrBind([at(ast, 2, 2)], env, [e]));
      //   }
      // } else if (ast[0] == "fn") {
      //   // define new function (lambda)
      //   // const f = (...a: any[]) => evalLoop(at(ast, 2), evalOrBind(at(ast, 1), env, a));
      //   const f = (...a: any[]) =>
      //     evalLoop(at(ast, 2), evalOrBind(at(ast, 1), env, a));
