...
`result.recover`, and basic object introspection. Remaining type-system work
includes richer exhaustiveness for lists, objects, redundant patterns, and
stdlib schemes for variadic, overloaded, and object/row-polymorphic builtins.
...
exported-only visibility, and cycle detection. The next module-system step is
CLI/proc-macro dependency tracking around the same resolver seam.
