...
| Cross-module, general compile-time macros | Current template macros are ordered and local by design. Local template-introduced bindings are hygienic, and exporting macros is now an expansion error across Lisp, JSON, and YAML. Imports are still not macro exports and the macro body is not a compile-time evaluator. | Explicit compile-time import form, evaluator capability/sandboxing rules if adopted, dependency/cycle diagnostics, and cross-module macro tests. |
| Full list/object exhaustiveness | The shipped contract now proves finite list lengths, exact finite-list products, nested finite object refinements, and finite object-field products up to the documented 256-combination cap. It deliberately does not attempt arbitrary list/object domains or guard-sensitive relational proofs. | Future expansion needs a general pattern-matrix design for arbitrary structural domains, richer missing-pattern diagnostics, and guard-sensitive reachability without weakening the current conservative guard rule. |
| Package registry tooling | Local path dependencies and lockfiles are supported. Registry dependency specs now have a documented manifest shape, source/index decision, checksum policy, and offline parser/error tests, but no fetch/cache/index implementation exists. | Registry index/cache implementation, lockfile entries for selected registry packages, checksum verification tests, and offline cache-hit tests. |
