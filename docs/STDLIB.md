...
| `ui.local` | `(A, (A, (A) -> ui.local-action) -> ui.node) -> ui.node` | Opt-in executor-owned state for one mounted component instance in an update-driven JUIR app; it is not supported by static `ui.html`. The callback binds `(state set-state)`; calling the setter from an event updates that instance without invoking the app reducer. Stable static component paths are supported; a changed dynamic `for` collection resets its local cells until key-aware ownership lands. | `(ui.local false (fn (open set-open) (button (on click (emit (set-open (not open)))) (text "Toggle"))))` |
