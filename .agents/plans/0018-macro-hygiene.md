...
templates. Unquoted arguments retain caller syntax, while literal identifiers
from a template can capture caller bindings. That is unacceptable once macros
become a reusable language seam.

## Proposed default

Hygiene applies to identifiers *introduced in binding positions by the
template*: `fn` parameters, `let` names, list-rest patterns, aliases, and
ordinary case-pattern bindings. Each expansion assigns a deterministic fresh
internal name and rewrites only references in that introduced lexical scope.
Unquoted and spliced caller nodes are never renamed. Core forms, prelude names,
known constructors, object keys, and import-qualified names are not bindings
and remain literal symbols.
...
- A template `let` binding cannot capture an unquoted caller identifier.
- A caller-supplied binding keeps its original spelling and scope.
...
- Lisp, JSON, and YAML normalize to the same hygienic expanded Core IR.
- Macro-import cycles and expansion-step limits produce source-ranged errors.
