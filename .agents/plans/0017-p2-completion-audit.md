...
| Cross-module, general compile-time macros | Current template macros are ordered and bounded to quote/quasiquote templates. Local template-introduced bindings are hygienic, exporting macros is an expansion error across Lisp, JSON, and YAML, path-aware `macro-import` can import file-module template macros under an alias, and macro-import source files are included in facade/native/proc-macro dependency tracking. Runtime imports are still not macro exports and the macro body is not a general compile-time evaluator. | Add richer macro-import dependency/cycle diagnostics and evaluator capability/sandboxing rules only if a future general compile-time evaluator is adopted. |
