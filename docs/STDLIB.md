...
| `ui.local` | `(A, (A, (A) -> ui.local-action) -> ui.node) -> ui.node` | Opt-in executor-owned state for one mounted component instance in an update-driven JUIR app; it is not supported by static `ui.html`. The callback binds `(state set-state)`; calling the setter from an event updates that instance without invoking the app reducer. Stable component paths and keyed `for` rows retain their cells through reordering; unkeyed dynamic rows reset on a collection change. | `(ui.local false (fn (open set-open) (button (on click (emit (set-open (not open)))) (text "Toggle"))))` |
| `ui.local.result` | `(A, list<ui.command>, list<ui.subscription>) -> ui.local-action` | Event-scoped local result: updates the owning `ui.local` state and replaces its complete declared resource snapshot. It is rejected from a non-local handler; completion actions go through the app reducer. | `(ui.local.result true (list (ui.command "save" "storage.write" 1 request false ok err)) (list))` |
