...
| Resolved export schema with explicit generic instantiations | `crates/jisp/tests/export_schema.rs` and `jisp export-schema --type`. |
...
| Rich recursive export-schema annotations | Explicit generic instantiation now has a public `--type` syntax and concrete generic tagged variants emit `$defs`. Recursive and mutually recursive generic annotations still need a deeper regression suite. | Recursive named-type fixtures, imported recursive generic types, and CLI/docs tests. |
