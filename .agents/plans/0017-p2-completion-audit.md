...
| Package registry tooling | Local path dependencies and lockfiles are supported. Registry dependency specs now have a documented manifest shape, source/index decision, checksum policy, an offline cache-hit path that verifies locked version/SHA-256 source checksums, and `jisp lock` preservation for used registry cache entries. Registry index lookup, downloads, and cache population do not exist yet. | Registry index/cache implementation, fetch/cache population tests, and end-to-end registry package tests. |
