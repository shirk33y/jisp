...
| Module graph, imports, stale tracking, and cycle checks | Ported for facade/type-checking imports, cycle detection, CLI dependency listing, proc-macro dependency tracking, and the supported native emission path; project-level stale tracking remains future work. | [`module_loader.rs`](https://github.com/gleam-lang/gleam/blob/833732c523441043868877d159988ba2d21538cd/compiler-core/src/build/module_loader.rs#L45-L84), [`project_compiler.rs`](https://github.com/gleam-lang/gleam/blob/833732c523441043868877d159988ba2d21538cd/compiler-core/src/build/project_compiler.rs#L105-L151), [`call_graph.rs`](https://github.com/gleam-lang/gleam/blob/833732c523441043868877d159988ba2d21538cd/compiler-core/src/call_graph.rs#L530-L544) | Directory-as-module loading needs deterministic resolution, useful cycle errors, and future incremental compilation. |
...
- guard-sensitive reachability beyond the current conservative rule that
  guarded branches do not establish coverage;
- overlap diagnostics from a general pattern-matrix checker beyond the current
  finite-domain redundancy checks;
- multi-subject `case`;
...
- Import/type-environment installation should stay explicit and module-path
  keyed. Jisp now has the facade/type-checking, CLI listing, proc-macro
  dependency tracking, and supported native-emission path on this seam.
...
- Already ported: ADT constructor schemes, top-level recursive SCC grouping,
  ...
  helpers.
- Still actionable in P2: richer `case` checking and structural object
  refinements.
- Later work: expand native token emission while continuing to consume the
  dependency information already exposed by the facade instead of adding a
